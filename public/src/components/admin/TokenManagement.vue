      ...
      <form @submit.prevent="handleAction">
        <div class="form-group-inline">
          ...
          <div class="form-group">
            ...
            <input type="number" id="amount" v-model.number="action.amount" placeholder="수량 입력" min="0" required />
          </div>
        </div>
        <div class="form-group">
          <label for="reason">사유</label>
          <input type="text" id="reason" v-model="action.reason" placeholder="발행 또는 소각 사유 (예: 이벤트 보상)" required />
        </div>
...
import { doc, getDoc, updateDoc, increment, serverTimestamp, writeBatch } from 'firebase/firestore';
...
const handleAction = async () => {
  ...
  if (!confirm(`${tokenName} 토큰 ${action.amount}개를 ${actionTypeText}하시겠습니까?`)) return;
  ...
  if (action.type === 'mint') {
    ...
    updatePayload[`${action.token}.circulatingSupply`] = increment(action.amount);
  } else { // burn
...
.token-card { padding: 20px; border-radius: 8px; color: white; }
.token-card h5 { border-bottom: 1px solid rgba(255,255,255,0.5); padding-bottom: 10px; margin-bottom: 15px; }
.token-card p { font-size: 1.1em; }
.cobs { background: linear-gradient(135deg, #2980b9, #3498db); }
.bnd { background: linear-gradient(135deg, #8e44ad, #9b59b6); }

/* 토큰 작업 폼 */
.token-actions .form-group-inline { display: flex; gap: 20px; margin-bottom: 20px; }
.token-actions .form-group { flex: 1; }
.token-actions label { display: block; margin-bottom: 8px; font-weight: 600; }
.token-actions input, .token-actions select { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box; }
