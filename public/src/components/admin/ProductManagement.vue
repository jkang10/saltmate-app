    ...
    <div class="product-form card">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <textarea v-model="currentProduct.description" rows="4" required></textarea>
        </div>
        <div class="form-group-inline">
          <div class="form-group">
            <label for="price">가격 (원)</label>
            <input type="number" v-model.number="currentProduct.price" required />
          </div>
          <div class="form-group">
            <label for="stock">재고</label>
            <input type="number" v-model.number="currentProduct.stock" required />
          </div>
        </div>
        <div class="form-group">
          ...
          <div v-if="isEditing && currentProduct.imageUrl" class="image-preview">
            <p>현재 이미지:</p>
      ...
      <table v-else-if="products.length > 0" class="product-table">
        ...
        <tbody>
          <tr v-for="product in products" :key="product.id">
            <td><img :src="product.imageUrl" alt="상품 이미지" class="table-img"/></td>
            <td>{{ product.name }}</td>
            <td>{{ formatCurrency(product.price) }}</td>
            <td>{{ product.stock }} 개</td>
            <td>
              <button @click="editProduct(product)" class="btn btn-sm btn-secondary">수정</button>
              <button @click="deleteProduct(product)" class="btn btn-sm btn-danger">삭제</button>
            </td>
          </tr>
        </tbody>
      </table>
...
const defaultProduct = { id: null, name: '', description: '', price: 0, stock: 0, imageUrl: '', imagePath: '' };
...
.product-form .form-group-inline { display: flex; gap: 20px; }
.product-form .form-group-inline .form-group { flex: 1; }
.product-form label { display: block; margin-bottom: 8px; font-weight: 600; }
.product-form input, .product-form textarea { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box; }
.image-preview { margin-top: 10px; }
.image-preview img { max-width: 200px; max-height: 200px; border: 1px solid #ddd; border-radius: 5px; }
.form-actions { display: flex; gap: 10px; }

.product-table { width: 100%; border-collapse: collapse; margin-top: 20px; }
.product-table th, .product-table td { border-bottom: 1px solid #eee; padding: 12px; text-align: left; vertical-align: middle; }
.product-table .table-img { width: 60px; height: 60px; object-fit: cover; border-radius: 5px; }
