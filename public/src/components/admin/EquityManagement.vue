      ...
      <table v-else-if="equityHolders.length > 0" class="equity-table">
        ...
        <tbody>
          <tr v-for="holder in equityHolders" :key="holder.userId">
            ...
            <td>
              <button @click="updateEquity(holder)" class="btn btn-sm btn-primary">
                지분율 저장
              </button>
            </td>
          </tr>
...
const totalEquity = computed(() => {
  return equityHolders.value.reduce((sum, holder) => sum + (holder.equityPercentage || 0), 0).toFixed(2);
});
...
const updateEquity = async (holder) => {
  ...
  if (!confirm(`'${holder.userName}'님의 지분율을 ${holder.newEquity}%로 변경하시겠습니까?`)) return;
...
.equity-table th, .equity-table td { border-bottom: 1px solid #eee; padding: 12px 15px; text-align: left; vertical-align: middle; }
.equity-input {
  width: 100px;
  padding: 8px;
