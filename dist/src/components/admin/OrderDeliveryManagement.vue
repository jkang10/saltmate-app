      ...
      <table v-else-if="orders.length > 0" class="order-table">
        ...
        <tbody>
          <tr v-for="order in orders" :key="order.id">
            ...
            <td>
              <select v-model="order.status" @change="updateStatus(order, $event.target.value)" class="status-select">
...
const getOrderItemsSummary = (items) => {
  ...
  return remainingCount > 0 ? `${firstItemName} 외 ${remainingCount}건` : firstItemName;
};
...
const updateStatus = async (order, newStatus) => {
  ...
  if (!confirm(`'${order.userName}'님의 주문 상태를 '${newStatusText}'(으)로 변경하시겠습니까?`)) {
